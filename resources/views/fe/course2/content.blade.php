<main class="col-span-12 md:col-span-8 lg:col-span-9">
    <div class="bg-white rounded-xl shadow-sm border">
        ...
        <div class="p-6 space-y-6 leading-relaxed">
            ...
            @if(isset($course) && $course->is_quiz)
                <div class="p-4 bg-yellow-50 rounded-lg">
                    ...
                    <form method="POST" action="{{ route('course2.quiz.check', $course->id) }}">
                        @csrf
                        ...
                        document.addEventListener("DOMContentLoaded", function () {
                            ...
                            if (result === "correct") {
                                ...
                                window.location.href =
                                    "{{ $nextCourse2 ? route('course2.show', $nextCourse2->slug) : route('course2.index') }}";
                            } else {
        ...
        <div class="p-6 flex justify-between border-t">
            {{-- Tombol Prev --}}
            @isset($prevCourse2)
                    <a href="{{ route('course2.show', $prevCourse2->slug) }}" class="px-4 py-2 rounded-lg transition
                                {{ in_array($prevCourse2->id, $completedCourses2)
            ...
            {{-- Tombol Next --}}
            @isset($nextCourse2)
                    <a href="{{ route('course2.show', $nextCourse2->slug) }}" class="px-4 py-2 rounded-lg transition
                                {{ in_array($nextCourse2->id, $completedCourses2)
                ? 'bg-blue-600 text-white hover:bg-blue-700'
