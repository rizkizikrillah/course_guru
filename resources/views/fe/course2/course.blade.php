...
<body class="bg-gray-100 text-gray-800">
    ...
    <div x-data="courseApp({{ $bab->count() }}, {{ json_encode($completedCourses2 ?? []) }})"
        ...
        {{-- SIDEBAR --}}
        @include('fe.course2.sidebar')

        {{-- MAIN CONTENT --}}
        @include('fe.course2.content')
    ...
    <script>
        function courseApp(totalBab, completedBab) {
            return {
                ...
                prevBab: @json($prevCourse2),
                nextBab: @json($nextCourse2),

                markComplete(babId) {
                    if (this.completedBab.includes(babId)) return;
                    fetch(`{{ url('course2') }}/${babId}/complete`, {
                        method: 'POST',
                        headers: {
                        ...
                        .then(data => {
                            ...
                            // Auto next
                            if (this.nextBab) {
                                window.location.href = `/course2/${this.nextBab.slug}`;
                            }
                        });
                },
                ...
                goPrev() {
                    if (this.prevBab) {
                        window.location.href = `/course2/${this.prevBab.slug}`;
                    }
                },
