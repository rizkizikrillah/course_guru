...
<body class="bg-gray-100 text-gray-800">
    ...
    <div x-data="courseApp({{ $bab->count() }}, {{ json_encode($completedCourses2 ?? []) }})"
        ...
        {{-- SIDEBAR --}}
        @include('fe.course3.sidebar')

        {{-- MAIN CONTENT --}}
        @include('fe.course3.content')
    ...
    <script>
        function courseApp(totalBab, completedBab) {
            return {
                ...
                prevBab: @json($prevcourse3),
                nextBab: @json($nextcourse3),

                markComplete(babId) {
                    if (this.completedBab.includes(babId)) return;
                    fetch(`{{ url('course3') }}/${babId}/complete`, {
                        method: 'POST',
                        headers: {
                        ...
                        .then(data => {
                            ...
                            // Auto next
                            if (this.nextBab) {
                                window.location.href = `/course3/${this.nextBab.slug}`;
                            }
                        });
                },
                ...
                goPrev() {
                    if (this.prevBab) {
                        window.location.href = `/course3/${this.prevBab.slug}`;
                    }
                },
