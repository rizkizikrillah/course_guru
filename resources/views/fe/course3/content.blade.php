<main class="col-span-12 md:col-span-8 lg:col-span-9">
    <div class="bg-white rounded-xl shadow-sm border">
        ...
        <div class="p-6 space-y-6 leading-relaxed">
            ...
            @if(isset($course) && $course->is_quiz)
                <div class="p-4 bg-yellow-50 rounded-lg">
                    ...
                    <form method="POST" action="{{ route('course3.quiz.check', $course->id) }}">
                        @csrf
                        ...
                        document.addEventListener("DOMContentLoaded", function () {
                            ...
                            if (result === "correct") {
                                ...
                                window.location.href =
                                    "{{ $nextcourse3 ? route('course3.show', $nextcourse3->slug) : route('course3.index') }}";
                            } else {
        ...
        <div class="p-6 flex justify-between border-t">
            {{-- Tombol Prev --}}
            @isset($prevcourse3)
                    <a href="{{ route('course3.show', $prevcourse3->slug) }}" class="px-4 py-2 rounded-lg transition
                                {{ in_array($prevcourse3->id, $completedCourses3)
            ...
            {{-- Tombol Next --}}
            @isset($nextcourse3)
                    <a href="{{ route('course3.show', $nextcourse3->slug) }}" class="px-4 py-2 rounded-lg transition
                                {{ in_array($nextcourse3->id, $completedCourses3)
                ? 'bg-blue-600 text-white hover:bg-blue-700'
