<main class="col-span-12 md:col-span-8 lg:col-span-9">
    <div class="bg-white rounded-xl shadow-sm border">
        ...
        <div class="p-6 space-y-6 leading-relaxed">
            ...
            @if(isset($course) && $course->is_quiz)
                <div class="p-4 bg-yellow-50 rounded-lg">
                    ...
                    <form method="POST" action="{{ route('coursegemini2.quiz.check', $course->id) }}">
                        @csrf
                        ...
                        document.addEventListener("DOMContentLoaded", function () {
                            ...
                            if (result === "correct") {
                                ...
                                window.location.href =
                                    "{{ $nextcourseGemini2 ? route('coursegemini2.show', $nextcourseGemini2->slug) : route('coursegemini2.index') }}";
                            } else {
        ...
        <div class="p-6 flex justify-between border-t">
            {{-- Tombol Prev --}}
            @isset($prevcourseGemini2)
                <a href="{{ route('coursegemini2.show', $prevcourseGemini2->slug) }}"
                    class="px-4 py-2 rounded-lg transition
                        {{ in_array($prevcourseGemini2->id, $completedCoursesGemini2)
                            ? 'bg-blue-600 text-white hover:bg-blue-700'
                            : 'bg-gray-200 text-gray-700 hover:bg-gray-300' }}">
                    ← {{ $prevcourseGemini2->title }}
                </a>
            @else
                <span></span>
            @endisset
            ...
            {{-- Tombol Next --}}
            @isset($nextcourseGemini2)
                <a href="{{ route('coursegemini2.show', $nextcourseGemini2->slug) }}"
                    class="px-4 py-2 rounded-lg transition
                        {{ in_array($nextcourseGemini2->id, $completedCoursesGemini2)
                            ? 'bg-blue-600 text-white hover:bg-blue-700'
                            : 'bg-gray-300 text-gray-600 hover:bg-gray-400' }}">
                    {{ $nextcourseGemini2->title }} →
                </a>
            @endisset
        </div>
    </div>
