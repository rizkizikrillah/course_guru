<main class="col-span-12 md:col-span-8 lg:col-span-9">
    <div class="bg-white rounded-xl shadow-sm border">
        ...
        <div class="p-6 space-y-6 leading-relaxed">
            ...
            @if(isset($course) && $course->is_quiz)
                <div class="p-4 bg-yellow-50 rounded-lg">
                    ...
                    <form method="POST" action="{{ route('coursebk.quiz.check', $course->id) }}">
                        @csrf
                        ...
                        document.addEventListener("DOMContentLoaded", function () {
                            ...
                            if (result === "correct") {
                                ...
                                window.location.href =
                                    "{{ $nextcoursebk ? route('coursebk.show', $nextcoursebk->slug) : route('coursebk.index') }}";
                            } else {
        ...
        <div class="p-6 flex justify-between border-t">
            {{-- Tombol Prev --}}
            @isset($prevcoursebk)
                    <a href="{{ route('coursebk.show', $prevcoursebk->slug) }}" class="px-4 py-2 rounded-lg transition
                                {{ in_array($prevcoursebk->id, $completedCourseBk)
            ...
            {{-- Tombol Next --}}
            @isset($nextcoursebk)
                    <a href="{{ route('coursebk.show', $nextcoursebk->slug) }}" class="px-4 py-2 rounded-lg transition
                                {{ in_array($nextcoursebk->id, $completedCourseBk)
                ? 'bg-blue-600 text-white hover:bg-blue-700'
